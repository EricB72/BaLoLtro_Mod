SMODS.Joker {
    ...
    calculate = function(self, card, context)
        ...
        if context.individual and context.cardarea == G.play then
            ...
            if numerator > 0 then
                return {
                    dollars = card.ability.extra.dollars * card.ability.extra.wildCards[numerator]
                }
            end
        end
    end,
        ...
        return {
            {
                ...
                ref_value = "totDollars"
            },
            text = {
                { text = "$", colour = G.C.GOLD},
                { ref_table = "card.joker_display_values", ref_value = "totDollars", retrigger_type = "mult", colour = G.C.GOLD }
            },
            reminder_text = {
                { text = '(' },
            ...
            calc_function = function(card)
                ...
                if text ~= "Unknown" and #scoring_hand > 0 then
                    ...
                    card.joker_display_values.totDollars = 1 * numerator
                else
                    card.joker_display_values.totDollars = 0
                end
            end
        }
    end
